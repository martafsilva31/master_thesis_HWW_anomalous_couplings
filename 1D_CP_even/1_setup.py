...
def setup_madminer(main_dir,plot_dir):
    """
    Sets up the MadMiner instance for WH signal, with only the CP-even operator (oHW), and morphing up to 2nd order.
    """
    ...
    miner.save(f'{main_dir}/setup_1D_CP_even.h5')

    morphing_basis=plot_1d_morphing_basis(miner.morpher,xlabel=r'$c_{HW}$',xrange=(-1.0,1.0))
    morphing_basis.savefig(f'{plot_dir}/morphing_basis_1D_CP_even.pdf')
