...
def evaluate_and_save_llr_individual(config):
  ...
  for i in range(5):
    ...
    parameter_grid,p_values,index_best_point,llr_kin,llr_rate, _ =limits_file.expected_limits(
    mode=config['limits']['mode'],theta_true=[0.0,0.0], include_xsec=not config['limits']['shape_only'],
    model_file = model_path if( 'ml' == config['limits']['mode'] or 'sally' == config['limits']['mode'] ) else None,
    luminosity=config['limits']['lumi']*1000.0,
    return_asimov=True,test_split=0.2,
    grid_ranges=config['limits']['grid_ranges'],grid_resolutions=config['limits']['grid_resolutions'])


    os.makedirs(f"{config['main_dir']}/llr_fits/{config['limits']['prior']}/{config['limits']['observables']}/{config['limits']['model']}/{config['sample_name']}/range_{config['limits']['grid_ranges']}_resolutions_{config['limits']['grid_resolutions']}_2D",exist_ok=True)
    save_dir = f"{config['main_dir']}/llr_fits/{config['limits']['prior']}/{config['limits']['observables']}/{config['limits']['model']}/{config['sample_name']}/range_{config['limits']['grid_ranges']}_resolutions_{config['limits']['grid_resolutions']}_2D"

    np.savez(f"{save_dir}/estimator_{i}_data_2D.npz", parameter_grid=parameter_grid, p_values=p_values,
...
def evaluate_and_save_llr_ensemble(config):
    ...
    np.savez(f"{save_dir}/ensemble_data_2D.npz", parameter_grid=parameter_grid, p_values=p_values,
                index_best_point=index_best_point, llr_kin=llr_kin, llr_rate=llr_rate)
