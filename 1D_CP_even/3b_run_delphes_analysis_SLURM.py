...
if __name__ == '__main__':
    ...
    benchmarks = ['morphing_basis_vector_1', 'morphing_basis_vector_2']
    
    if args.do_signal:
        for sample in signal_samples:
            sample_folder=f'{main_dir}/signal_samples/{sample}_smeftsim_SM/'
            run_delphes_analysis_SLURM(sample_folder,main_dir, setup_file,args.do_delphes,delphes_card,'sm')
    

    if args.do_backgrounds:
        for sample in background_samples:
            sample_folder=f'{main_dir}/background_samples/{sample}_background/'
            run_delphes_analysis_SLURM(sample_folder,main_dir,setup_file,args.do_delphes,delphes_card, 'sm')
    ...
    if args.do_bsm:
        for sample in signal_samples:
            for bench in benchmarks:
                sample_folder=f'{main_dir}/signal_samples/{sample}_smeftsim_{bench}/'
