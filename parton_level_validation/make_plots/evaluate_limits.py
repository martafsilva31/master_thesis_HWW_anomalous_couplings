...
def evaluate_and_save_llr_individual(config):
  ...
  for i in range(5):
    ...
    # Check if binning_x is provided in the config
    if config['limits']['binning_x'] is not None:
        # Check if the binning is a list of values or just the number of bins
        if len(config['limits']['binning_x']) != 1:
            hist_bins = [config['limits']['binning_x']]  # Use the binning values provided
        else:
            hist_bins = [int(config['limits']['binning_x'][0])]  # Use the number of bins
...
def plot_llr_individual(config):
    ...
    for i in range(5):
      ...
      dir =  load_dir + f"/estimator_{i}_data_test.npz"
