...
def evaluate_limits_histo(config):
    ...
    save_dir = f"{config['main_dir']}/llr_fits_hist/{vars[0]}/{config['sample_name']}/range_{config['limits']['grid_ranges']}_resolutions_{config['limits']['grid_resolutions']}_2D"
    os.makedirs(save_dir, exist_ok=True)

    np.savez(f"{save_dir}/data_2D.npz", parameter_grid=parameter_grid, p_values=p_values,
...
if __name__ == "__main__":
    config = {
        ...
        'sample_name': 'wh_signalWithBSMAndBackgrounds_2D',
        'limits': {
            'lumi': 300,
