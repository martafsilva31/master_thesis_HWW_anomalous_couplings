...
if __name__ == "__main__":
        ...
        for channel in channels:
            for run in os.listdir(f'{main_dir}/signal_samples/{channel}_smeftsim_SM/Events'):
                miner.reweight_existing_sample(
                    ...
                    run_name=run,
                    sample_benchmark='sm',
                    reweight_benchmarks= ['morphing_basis_vector_1','morphing_basis_vector_2'], 
    ...
    if args.do_bsm:
        for channel in channels:
            for run in os.listdir(f'{main_dir}/signal_samples/{channel}_smeftsim_morphing_basis_vector_1/Events'):
                miner.reweight_existing_sample(
                    mg_process_directory=f'{main_dir}/signal_samples/{channel}_smeftsim_morphing_basis_vector_1',
                    run_name=run,
                    sample_benchmark='morphing_basis_vector_1',
                    reweight_benchmarks= ['sm', 'morphing_basis_vector_2'], 
                    param_card_template_file=param_card_template_file,
                    log_directory=f'{main_dir}/logs/{channel}_smeftsim_morphing_basis_vector_1_reweight/{run}',
                )

            for run in os.listdir(f'{main_dir}/signal_samples/{channel}_smeftsim_morphing_basis_vector_2/Events'):
                miner.reweight_existing_sample(
                    mg_process_directory=f'{main_dir}/signal_samples/{channel}_smeftsim_morphing_basis_vector_2',
                    run_name=run,
                    sample_benchmark='morphing_basis_vector_2',
                    reweight_benchmarks= ['sm', 'morphing_basis_vector_1'], 
                    param_card_template_file=param_card_template_file,
                    log_directory=f'{main_dir}/logs/{channel}_smeftsim_morphing_basis_vector_2_reweight/{run}',
                )
